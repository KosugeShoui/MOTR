...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'visem/val', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'visem/val', self.seq_num, 'img1', _) for _ in img_list if
        ...
        self.save_path = os.path.join(self.args.output_dir, 'eval/results/{}'.format(seq_num))
        os.makedirs(self.save_path, exist_ok=True)

        self.predict_path = os.path.join(self.args.output_dir, 'eval/preds', self.seq_num)
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'visem/val')
    ...
    def detect(self, prob_threshold=0.4, area_threshold=100, vis=False):
        total_dts = 0
...
if __name__ == '__main__':
    ...
    print(strsummary)
    with open(args.output_dir + "/eval/eval_log.txt", 'a') as f:
