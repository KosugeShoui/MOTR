...
def draw_points(img: np.ndarray, points: np.ndarray, color=(255, 255, 255)) -> np.ndarray:
    ...
    for i, (x, y) in enumerate(points):
        if i >= 300:
            color = (0, 255, 0)
        cv2.circle(img, (int(x), int(y)), 2, color=color, thickness=1)
    return img
...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'visem/train', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'visem/train', self.seq_num, 'img1', _) for _ in img_list if
        ...
        self.save_path = os.path.join(self.args.output_dir, 'eval_train/results/{}'.format(seq_num))
        os.makedirs(self.save_path, exist_ok=True)

        self.predict_path = os.path.join(self.args.output_dir, 'eval_train/preds', self.seq_num)
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'visem/train')
    ...
    def detect(self, prob_threshold=0.1, area_threshold=30, vis=False):
        total_dts = 0
...
if __name__ == '__main__':
    ...
    val_path = args.mot_path + '/visem/train'
    ...
    print(strsummary)
    with open(args.output_dir + "/eval_train/eval_log.txt", 'a') as f:
